...
void MatrixType(std::string fname,unsigned char &mtype,unsigned char &ctype,unsigned char &endianness,unsigned char &mdinf,indextype &nrows,indextype &ncols)
{
 ...
 if (t == nullptr)
  JMatrixStop("You system cannot book space for a variable of 'indextype' (currently, unsigned int) aligned to a boundary of indextype size.\n");
...
{
 if ((unsigned int)cnames.size() != nc)
